...
from .prompt_format import (
    _escape_user_input,
    _format_company_brief,
    _format_decision_card,
    _format_employees,
    _format_gossip_leads,
    _format_history,
...
PROMPT_TEMPLATE_DIRECTOR = "\n".join(
    [
        ...
        "如果输入不足以判断，只能在允许枚举内给出保守结论，不得新增字段或输出解释。",
        "【约束】",
        ...
        "- employeeGossip.mood 必须是 anxious / angry / tired / hopeful / numb / excited "
        "/ in_love / neutral 之一",
        "- mediaHeadline.outlet 只能从指定池中选",
        "- mediaHeadline.headline ≤ 22 字",
        "- mediaHeadline.tone 必须是 positive / neutral / negative / mocking 之一",
        "- rivalAction.action 必须是 price_war / poach / launch / pr_attack / wait "
        "/ acquisition_rumor 之一",
        ...
        "不得包含 comments、analysis、reason、warning 或任何 schema 外字段。",
...
PROMPT_TEMPLATE_PRESS_EVAL = "\n".join(
    [
        ...
        "如果 transcript 没有可提取金句，memorableQuote 必须返回空字符串或原文中",
        "最短可引用句，不得编造。",
        "【约束】",
        ...
        "mediaAngle、metricsDelta、internalEval。不得包含 schema 外字段。",
...
PROMPT_TEMPLATE_DEATH_REPORT = "\n".join(
    [
        ...
        "如果没有可靠员工信息，lastEmployee 必须是 null；如果没有可验证决策 ID，",
        "biggestMistakeDecisionId 必须是 null。",
        "【约束】",
        "- obituary: 字符串，300-500 字中文",
        "- biggestMistakeDecisionId: 字符串或 null；非 null 时必须是输入中出现过的决策 ID",
        "- lastEmployee: 对象或 null；对象仅允许 name、quote 两个字符串字段",
        ...
        "lastEmployee、headlines、legacyUnlocks。不得包含 schema 外字段。",
        "严格 JSON，不要 markdown 包裹。",
        "【示例输出】",
    ...
    def build_director_prompt(self, ctx: Any) -> PromptBundle:
        user = "\n".join(
            [
                ...
                "公司画像:",
                _format_company_brief(ctx.company_brief),
                "当前 4 指标:",
                ...
                "当季已收集员工八卦:",
                _format_gossip_leads(ctx.gossip_collected),
                "员工快照（已移除 hidden_secrets）:",
                _format_employees(ctx.employees_snapshot),
                ...
                f"meta_buff_signature: {_escape_user_input(ctx.meta_buff_signature) or 'none'}",
            ]
        )
